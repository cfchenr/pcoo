...
public class programTest extends PApplet {
    ...
    public void draw() {
        ...
        for (int i = 0; i < cco.gTransports().size(); i++)
            if (cco.gTransports().get(i).gState() == "in transit")
                text(cco.gTransports().get(i).gNumber() + ": " + cco.gTransports().get(i).gSource().gName() + " -> "
        ...
        for (int i = 0; i < cco.gInfrastructures().size(); i++) {
            ...
            terminalButton.add(new Button(8 * width / 34 + (i % 3) * (8 * width / 34) + 5,
                    height / 14 + line * (3 * height / 14) + (height / 38), cco.gInfrastructures().get(i).gName()));
            terminalButton.get(i).draw();

            text("Máquinas a carregar", 8 * width / 34 + (i % 3) * (8 * width / 34) + 5,
            ...
            text("Máquinas a descarregar", 8 * width / 34 + (i % 3) * (8 * width / 34) + 5,
            ...
            text("Máquinas estacionadas ou em espera", 8 * width / 34 + (i % 3) * (8 * width / 34) + 5,
            ...
            for (int j = 0; j < cco.gTransports().size(); j++)
                if (cco.gTransports().get(j).gState() != "in transit"
                    ...
                    text(cco.gTransports().get(j).gNumber(),
        ...
        for (int i = 0; i < terminalButton.size(); i++)
            if (terminalButton.get(i).gDisplay()) {
                ...
                textAlign(PConstants.LEFT);
                text("Containers para carregar", width / 34 + 5, height / 14 - 5);
                ...
                text("Containers descarregados", 17 * width / 34 + 5, height / 14 - 5);
    ...
    public static ArrayList<Container> createContainers(Terminal terminal, int size, Terminal[] destinations) {
        ...
        for (int j = 0; j < size; j++) {
            ...
            containers.add(new Container(++contentorNumber, destinations[destination]));
        }
